...
description: Lancer, voir et piloter TryCast dans le simulateur iOS — boot du simulateur, Metro/Expo Go, screenshots lisibles par Claude, arbre d'accessibilité, taps/saisie via AXe, navigation par deep link Expo Router. À utiliser dès qu'il faut vérifier visuellement un écran, tester un parcours dans l'app, ou reproduire un bug UI en conditions réelles.
---

# Piloter TryCast dans le simulateur iOS

Workflow validé le 10/07/2026 (Xcode 26.6, simulateurs iOS 26.5, AXe 1.7.1). L'app tourne dans **Expo Go** (pas de dev build). Tout se fait en CLI, aucune app MCP requise.
...
npx expo start --ios --port 8081 # EN TÂCHE DE FOND (Bash run_in_background) — installe Expo Go si absent et ouvre l'app
```

Attendre que la sortie Metro contienne `iOS Bundled` (boucle `until grep -q "iOS Bundled" <fichier-output>` en background — jamais de `sleep` long en avant-plan). Si Metro tourne déjà, ouvrir l'app avec `xcrun simctl openurl booted exp://localhost:8081`.
...
4. **Deep links Expo Router** : `xcrun simctl openurl booted "exp://localhost:8081/--/<route>"` (ex. `league/join`, `settings`, `leaderboard`). ⚠️ Provoque un **rechargement complet du bundle** (~5-10 s de splash) avant d'arriver sur l'écran — attendre, puis vérifier par screenshot ou describe-ui. Limites constatées : un écran ouvert ainsi n'a **pas de pile derrière lui** (l'edge-swipe retour ne dépile rien) et le deep link vers la racine (`/--/` ou URL sans chemin) **ne re-navigue pas** si l'app est déjà ouverte. Pour revenir à l'accueil de façon fiable : `xcrun simctl terminate booted host.exp.Exponent && xcrun simctl openurl booted exp://localhost:8081`.
...
- **HMR** : après une édition de code, Expo Go recharge tout seul (Fast Refresh) — pas besoin de relancer, juste re-screenshoter. Rechargement forcé : `r` impossible (Metro en background) → refaire `openurl exp://localhost:8081`.
