...
Les deux boutons portent un **emoji, pas un logo** (« 🍏 Sur iPhone », « 🤖 Sur Android », choix de Corentin) : le logo Apple est interdit aux tiers, le robot Android exigerait une mention d'attribution, et Proton comme Outlook bloquent les images distantes par défaut. Les étapes du tuto citent les libellés **sans emoji**, exprès : ne pas les « aligner » sur les boutons.

**Un broadcast Resend, pas un template.** Il n'accepte que des variables par contact (`FIRST_NAME`, `EMAIL`, `RESEND_UNSUBSCRIBE_URL`) : le lien TestFlight, le même pour tous, n'a pas de variable et s'inscrit dans le HTML par `--testflight`. Un broadcast ne se crée qu'avec un **segment**, donc pas avant l'import de l'Audience des testeurs. Déroulé : Corentin importe l'Audience et donne le vrai lien TestFlight ; Claude génère l'e-mail (`--testflight`) et crée le broadcast **en brouillon** par le MCP Resend (`create-broadcast` avec le segment, puis `update-broadcast` avec le HTML) ; Corentin fait l'envoi test, puis envoie. Claude n'envoie jamais un broadcast.
