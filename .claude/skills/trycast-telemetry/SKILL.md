...
Les 15 événements actuels : `account_created`, `signed_in`, `prediction_saved` (`first`
...
vs. passé ou glissé), `feedback_sent` (`withEmail`, jamais le contenu du message), `reaction_changed` (`action` : `set` / `changed` / `cleared`, et `reaction` parmi les quatre clés — jamais la ligue, le match ni la personne visée), `round_highlight_opened` (`from` : `card` / `push`, carte du coup de la journée ouverte vers le match — jamais la ligue ni le lauréat).
