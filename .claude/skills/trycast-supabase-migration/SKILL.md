...
- **Un `check … not valid` ne dispense pas les lignes existantes** : il saute la vérification au moment de l'`alter`, mais Postgres le revérifie à **chaque** `update` de la ligne, même sur une autre colonne. Une ligne non conforme devient donc impossible à modifier (23514). Vécu le 2026-09-26 avec `profiles_username_clean` : le profil de « DemoTryCast », le compte des relecteurs des stores, refusait la langue synchronisée au lancement et la photo (correctif `20260926000300`, `is_demo or username_is_clean(username)`). Réflexe : avant d'ajouter un `check not valid`, lister les lignes non conformes **sur chaque projet** et décider de leur sort (exemption dans la contrainte ou correction), sans compter sur le `not valid`.
...
- E-mail de bienvenue : `supabase db query --linked -f scripts/e2e-welcome.sql`, **sans seed**, même modèle, une fois `20260926000400` poussée (un envoi au premier `claim_username` d'un compte sans pseudo, aucun au renommage ni pour un compte de démo ; l'assertion lit la requête en file dans `net.http_request_queue`, rien ne part). Pour vérifier une migration **avant** son push, la coller en tête d'une copie du script : elle est annulée avec le reste.
