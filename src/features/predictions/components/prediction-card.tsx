...
export function PredictionCard({ match, prediction, userId, distribution }: PredictionCardProps) {
    ...
    // Aperçu du bonus offensif indexé sur la cote de victoire de CHAQUE équipe
    // (pas celle du vainqueur), calculable dès qu'on a les cotes.
    ...
    return (
        <View className="gap-3.5 rounded-lg border border-border bg-surface p-4 tc-shadow-md">
            ...
            {/* Bonus offensif : gain indexé sur la cote de l'équipe, risque de malus si raté */}
            <View className="flex-row items-center justify-center gap-3">
                <View className="w-12 items-center">
                    <Text className="font-display text-[16px] text-accent" numberOfLines={1}>
                        {`+${bonusHomeValue}`}
                    </Text>
                    <Text className="font-body-bold text-[10px] text-danger" numberOfLines={1}>
                        {`−${rules.offensiveMalusPoints}`}
                    </Text>
                </View>
                <BonusToggle
