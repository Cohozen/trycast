...
-- exécution.
--
-- Points attendus sur le match -103 (24-17, cotes 1.5/21/2.8, barème v2) :
--   user1 (20-10 + bonus off domicile) : vainqueur 15×1.5=23, écart prédit 10
--   vs réel 7 → ±5 = +8, bonus off en attente → 31 pts, puis 37 en passe 2
--   (essais 4-1 : bonus off domicile +0.25×15×1.5 = 6)
--   user2 (10-20) : mauvais vainqueur → 0 pt
-- Points attendus sur le match -104 (30-3, cotes 1.2/15/5.0) :
--   user1 (30-3) : vainqueur 15×1.2=18, score exact +50 → 68 pts
-- Standings attendus (compétition e2e-test) :
--   après passe 1 -103 : user1 (31, 1, 0), user2 (0, 1, 0)
--   après passe 2 -103 : user1 (37, 1, 0)
--   après scoring -104 : user1 (105, 2, 1), user2 inchangé
...
begin
    ...
    -- Payloads tels que les construirait l'EF (buildScoringPayload), barème v2
    v_pass1 := jsonb_build_array(
        jsonb_build_object(
            'prediction_id', v_p1, 'points_awarded', 31,
            'points_breakdown', '{"predictedOutcome":"home","winnerCorrect":true,
                "oddsUsed":1.5,"oddsFallback":false,"winnerPoints":23,
                "exactScorePoints":0,"gapPoints":8,"defensiveBonusPoints":0,
                "offensiveHome":{"checked":true,"oddsUsed":1.5,"oddsFallback":false,"tries":null,"pending":true,"points":0},
                "offensiveAway":{"checked":false,"oddsUsed":2.8,"oddsFallback":false,"tries":null,"pending":false,"points":0},
                "offensiveBonusPending":true}'::jsonb
        ),
        jsonb_build_object(
            ...
            'points_breakdown', '{"predictedOutcome":"away","winnerCorrect":false,
                ...
                "offensiveHome":{"checked":false,"oddsUsed":1.5,"oddsFallback":false,"tries":null,"pending":false,"points":0},
                "offensiveAway":{"checked":false,"oddsUsed":2.8,"oddsFallback":false,"tries":null,"pending":false,"points":0},
                "offensiveBonusPending":false}'::jsonb
        )
    );

    -- Passe 1 : points écrits, match horodaté
    perform public.apply_match_scores(v_match, 2, v_pass1);
    ...
    assert v_points1 = 31, format('user1 : 31 pts attendus, %s obtenus', v_points1);
    assert v_points2 = 0, format('user2 : 0 pt attendu, %s obtenus', v_points2);
    assert (select scoring_rule_version from public.predictions where id = v_p1) = 2,
        'scoring_rule_version 2 non enregistré';
    ...
    raise notice 'OK passe 1 : points écrits (user1=31, user2=0), match horodaté';
    ...
    assert v_standing = (31, 1, 0),
        format('standings user1 passe 1 : (31,1,0) attendu, %s obtenu', v_standing);
    select total_points, predictions_scored, exact_scores into v_standing
    ...
    raise notice 'OK standings passe 1 : user1 (31,1,0), user2 (0,1,0)';

    -- Idempotence : rejouer le même payload reproduit le même état
    perform public.apply_match_scores(v_match, 2, v_pass1);
    select points_awarded into v_points1 from public.predictions where id = v_p1;
    assert v_points1 = 31, 'rejeu : points user1 modifiés';
    assert (select count(*) from public.predictions where match_id = v_match) = 2,
        'rejeu : nombre de pronos modifié';
    select total_points, predictions_scored, exact_scores into v_standing
        from public.standings where user_id = v_u1 and competition_id = v_comp;
    assert v_standing = (31, 1, 0), 'rejeu : standings user1 modifiés';
    ...
    begin
        perform public.apply_match_scores(v_match, 2, jsonb_build_array(v_pass1 -> 0));
    ...
    if v_match_futur is not null then
        begin
            perform public.apply_match_scores(v_match_futur, 2, '[]'::jsonb);
            raise exception 'ECHEC_ATTENDU';
        exception when others then
    ...
    where id = v_match;
    v_pass2 := jsonb_set(
        jsonb_set(v_pass1, '{0,points_awarded}', '37'),
        '{0,points_breakdown}',
        '{"predictedOutcome":"home","winnerCorrect":true,
          "oddsUsed":1.5,"oddsFallback":false,"winnerPoints":23,
          "exactScorePoints":0,"gapPoints":8,"defensiveBonusPoints":0,
          "offensiveHome":{"checked":true,"oddsUsed":1.5,"oddsFallback":false,"tries":4,"pending":false,"points":6},
          "offensiveAway":{"checked":false,"oddsUsed":2.8,"oddsFallback":false,"tries":1,"pending":false,"points":0},
          "offensiveBonusPending":false}'::jsonb
    );
    perform public.apply_match_scores(v_match, 2, v_pass2);
    select points_awarded into v_points1 from public.predictions where id = v_p1;
    assert v_points1 = 37, format('passe 2 user1 : 37 pts attendus, %s obtenus', v_points1);
    assert (select points_breakdown ->> 'offensiveBonusPending'
            from public.predictions where id = v_p1) = 'false',
        'passe 2 : bonus offensif toujours en attente';
    select total_points, predictions_scored, exact_scores into v_standing
        from public.standings where user_id = v_u1 and competition_id = v_comp;
    assert v_standing = (37, 1, 0),
        format('standings user1 passe 2 : (37,1,0) attendu, %s obtenu', v_standing);
    raise notice 'OK passe 2 : bonus offensif crédité (37 pts), standings (37,1,0)';
    ...
    perform public.apply_match_scores(v_match2, 2, jsonb_build_array(
        jsonb_build_object(
            'prediction_id', v_p3, 'points_awarded', 68,
            'points_breakdown', '{"predictedOutcome":"home","winnerCorrect":true,
                "oddsUsed":1.2,"oddsFallback":false,"winnerPoints":18,
                "exactScorePoints":50,"gapPoints":0,"defensiveBonusPoints":0,
                "offensiveHome":{"checked":false,"oddsUsed":1.2,"oddsFallback":false,"tries":2,"pending":false,"points":0},
                "offensiveAway":{"checked":false,"oddsUsed":5.0,"oddsFallback":false,"tries":0,"pending":false,"points":0},
                "offensiveBonusPending":false}'::jsonb
        )
    ));
    select total_points, predictions_scored, exact_scores into v_standing
        from public.standings where user_id = v_u1 and competition_id = v_comp;
    assert v_standing = (105, 2, 1),
        format('standings user1 après -104 : (105,2,1) attendu, %s obtenu', v_standing);
    select total_points, predictions_scored, exact_scores into v_standing
        from public.standings where user_id = v_u2 and competition_id = v_comp;
    assert v_standing = (0, 1, 0),
        format('standings user2 après -104 : (0,1,0) attendu, %s obtenu', v_standing);
    raise notice 'OK match -104 : agrégat multi-matchs (105,2,1), exact_scores compté, user2 intact';
