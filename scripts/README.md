...
| `e2e-welcome.sql` | E-mail de bienvenue : un envoi au premier `claim_username` d'un compte sans pseudo (template `welcome`, destinataire, pseudo), aucun au renommage ni pour un compte de démo. `supabase db query --linked -f scripts/e2e-welcome.sql`, après le push de `20260926000400` | Aucun : transaction autonome, annulée à la fin (rien ne part) ; secret `resend_api_key` requis |
