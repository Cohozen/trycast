...
- Templates d'e-mails : `supabase/templates/*.html` sont **générés** par `scripts/build-email-templates.mjs` (`npm run emails:build`, `npm run emails:check`) — ne jamais les éditer à la main. ⚠️ **`supabase config push` pousse toute la section `[auth]`** et le `config.toml` du repo n'est pas un miroir du dashboard (rate limit, `site_url`, SMTP) : le pousser tel quel casserait la config Resend. Contraintes du HTML d'e-mail et pièges : skill `trycast-emails`
