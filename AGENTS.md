...
- **Joueur bloqué** (`apps/mobile/src/features/profile/`) : masqué **côté client** en « Joueur masqué » sans photo, rang conservé — **tout hook qui montre des joueurs passe son résultat par `useMaskBlocked`** en `select` ; ses réactions sont filtrées **côté serveur** (`user_blocks` dans les RPC de lecture), et seul l'écran « Joueurs bloqués » montre le vrai pseudo (skill `trycast-regles-metier`)
...
- **Apple en service sur iOS seulement**, en premier dans la liste : flux natif (`apple-sign-in.ts`), SHA-256 du nonce envoyé à Apple, nonce brut à `signInWithIdToken`, portée `EMAIL` seule. **Pas d'Apple sur Android, exprès** (le flux navigateur exigerait un Services ID et un secret à renouveler tous les six mois). Seul `ERR_REQUEST_CANCELED` est un renoncement : toute autre erreur s'affiche. **À la suppression du compte, le jeton Apple est révoqué** (règle 5.1.1(v), contrôlée en revue) : `useDeleteAccount` rouvre la feuille Apple pour tout fournisseur marqué `revokeOnDeletion` dans `providers.ts`, et l'EF `delete-account` échange puis révoque le code. Ne jamais appeler l'EF sans ce passage
